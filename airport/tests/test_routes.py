...
class RouteAPITestCase(TestCase):
    ...
    def get_list_url(self):
        return reverse("airport:route-list")
    ...
    def test_retrieve_route_authenticated(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(self.get_detail_url(self.route.id))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_route_as_user_forbidden(self):
        ...
        response = self.client.put(self.get_detail_url(self.route.id), data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_delete_route_as_user_forbidden(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(self.get_detail_url(self.route.id))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_filter_routes_by_source_name(self):
        self.client.force_authenticate(user=self.user)
        url = self.get_list_url() + "?source=lax"
    ...
    def test_filter_routes_by_destination_name(self):
        self.client.force_authenticate(user=self.user)
        url = self.get_list_url() + "?destination=sfo"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
    ...
    def test_filter_routes_by_min_distance(self):
        self.client.force_authenticate(user=self.user)
        url = self.get_list_url() + "?min_distance=500"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_filter_routes_by_max_distance(self):
        self.client.force_authenticate(user=self.user)
        url = self.get_list_url() + "?max_distance=600"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
